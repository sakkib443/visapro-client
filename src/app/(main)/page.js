...
const ICON_MAP = {
  LuPlane, LuCompass, LuGlobe, LuMap, LuShieldCheck,
  LuRocket, LuTarget, LuAward, LuTag, LuAccessibility,
  LuMousePointerClick,
};
...
export default function HomePage() {
  ...
  // ─── Partners Data ───
  const partnersData = homeData?.partners;
  const partnerItems = partnersData?.items || [
    { name: "G-FLY GLOBAL", icon: "LuPlane", color: "primary" },
    { name: "TRAVERSE", icon: "LuCompass", color: "secondary" },
    { name: "TRIPZONE", icon: "LuGlobe", color: "#3590CF" },
    { name: "BORCELLE", icon: "LuMap", color: "red" },
    { name: "TRAVEL TRUST", icon: "LuShieldCheck", color: "green" },
  ];
  ...
  // ─── Partner icon color map ───
  const getPartnerColorClass = (color) => {
    const map = {
      primary: { bg: "bg-primary/10", text: "text-primary" },
      secondary: { bg: "bg-secondary/10", text: "text-secondary" },
      "#3590CF": { bg: "bg-[#3590CF]/10", text: "text-[#3590CF]" },
      red: { bg: "bg-red-100", text: "text-red-600" },
      green: { bg: "bg-green-100", text: "text-green-600" },
    };
    return map[color] || { bg: "bg-gray-100", text: "text-gray-600" };
  };

  const getPartnerIcon = (iconName) => {
    const Icon = ICON_MAP[iconName];
    return Icon ? <Icon className="w-6 h-6" /> : <LuGlobe className="w-6 h-6" />;
  };
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      {(!partnersData || partnersData.isActive !== false) && (
              ...
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="flex items-center gap-12">
                  {partnerItems.map((p, idx) => {
                    const colors = getPartnerColorClass(p.color);
                    return (
                      <div key={`${i}-${idx}`} className="flex items-center gap-3 transition-all cursor-default group">
                        <div className={`w-10 h-10 ${colors.bg} rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform`}>
                          <span className={colors.text}>{getPartnerIcon(p.icon)}</span>
                        </div>
                        <span className="text-xl font-bold text-gray-900 uppercase" style={{ fontFamily: 'Teko, sans-serif' }}>{p.name}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              >
                ...
                <div className="flex items-center justify-center lg:justify-start gap-3 mb-6">
                  ...
                  <div className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#EF8C2C]/15 bg-[#EF8C2C]/[0.05]">
                    <span className="text-[#EF8C2C] text-[9px] lg:text-[10px] font-semibold tracking-[0.2em] lg:tracking-[0.25em] uppercase" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                      {cd ? bt(cd.tagText) : t('consultTag')}
                    </span>
                  </div>
                ...
                <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-[#0F172A] leading-[1] mb-6 tracking-tight" style={{ fontFamily: headingFont }}>
                  {cd ? bt(cd.heading) : t('consultTitle')}<br />
                  <span className="text-[#1D7EDD]">{cd ? bt(cd.headingHighlight) : t('consultTitleHighlight')}</span> {cd ? bt(cd.headingEnd) : t('consultTitleEnd')}
                </h2>
                ...
                <p className="text-gray-400 text-sm mb-8 mx-auto lg:mx-0 max-w-lg leading-relaxed" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                  {cd ? bt(cd.description) : t('consultDesc')}
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center lg:items-start gap-5 mb-10 p-5 rounded-2xl bg-[#F8F9FA] border border-gray-100">
                  <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0">
                    <img
                      src={cd?.experienceImage || "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=200&h=200&fit=crop"}
                      alt="Experience"
                ...
                <div className="flex flex-wrap items-center gap-5">
                  <Link href={cd?.ctaLink || "/contact"}>
                    ...
                    >
                      <span className="text-xs uppercase tracking-[0.1em]" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                        {cd ? bt(cd.ctaText) : t('exploreMore')}
                      </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-[#0F172A]" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                      {cd?.agentCount || "200+"} <span className="text-gray-400 font-normal">{cd ? bt(cd.agentLabel) : t('realAgents')}</span>
                    </span>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <div className="relative min-h-[350px] sm:min-h-[420px] lg:min-h-[460px]">
                  <div className="absolute top-0 left-0 w-[45%] sm:w-[35%] h-[350px] sm:h-[450px] overflow-visible z-20">
                    <img
                      src={cd?.mainImage2 || "/images/img02.png"}
                  ...
                  <div className="absolute top-[30px] lg:top-[30px] right-0 w-[75%] sm:w-[85%] h-[280px] sm:h-[380px] lg:h-[420px] rounded-2xl overflow-hidden shadow-xl z-10">
                    <img
                      src={cd?.mainImage1 || "/images/img01.png"}
          ...
          <div className="max-w-6xl mx-auto relative z-10">
              ...
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <div className="flex items-center gap-2 px-5 py-2 rounded-full border border-[#EF8C2C]/15 bg-[#EF8C2C]/[0.05]">
                  <span className="text-[#EF8C2C] text-xs font-semibold tracking-[0.25em] uppercase" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                    {wc ? bt(wc.tagText) : t('whyVisaPro')}
                  </span>
                </div>
                <div className="h-[1px] w-12 bg-gradient-to-l from-transparent to-[#EF8C2C]" />
              </div>
              <h2 className="text-3xl md:text-6xl font-bold mb-4" style={{ fontFamily: headingFont, color: '#111827', textTransform: 'uppercase' }}>
                {wc ? bt(wc.heading) : t('whyChoose')} <span style={{ color: '#EF8C2C' }}>{wc ? bt(wc.headingHighlight) : t('us')}</span>
              </h2>
              <p className="text-gray-400 text-sm max-w-xl mx-auto leading-relaxed" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>
                {wc ? bt(wc.description) : t('whyChooseDesc')}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {whyChooseCards.map((item, i) => (
                ...
                >
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-5 transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: `${item.color}10` }}>
                    {item.icon}
                  </div>
                  <h4 className="text-xl font-bold text-[#0F172A] mb-2" style={{ fontFamily: headingFont, textTransform: 'uppercase' }}>{item.title}</h4>
                  <p className="text-gray-400 text-sm leading-relaxed" style={{ fontFamily: bnFont || 'Poppins, sans-serif' }}>{item.desc}</p>
                </motion.div>
              ))}
            </div>
